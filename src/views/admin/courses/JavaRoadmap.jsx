...
import javaQuizzes from '../../../data/javaQuizzes';
import javaAssignments from '../../../data/javaAssignments';

// Transform roadmap data to include quiz and assignment data
const javaRoadmapData = {
  nodes: javaRoadmap.nodes.map(node => {
    const transformedNode = { ...node };
    
    // Add quiz data if it's a quiz node
    if (node.data.type === 'quiz') {
      const quizId = node.id;
      if (javaQuizzes[quizId]) {
        transformedNode.data.quiz = {
          questions: javaQuizzes[quizId].questions.map(q => ({
            question: q.question,
            options: q.options,
            correct: q.correct,
    ...
    if (node.data.type === 'assignment') {
      ...
      if (javaAssignments[assignmentId]) {
        const assignment = javaAssignments[assignmentId];
  ...
  edges: javaRoadmap.edges
};

const JavaRoadmap = () => {
  // Transform nodes to use default React Flow node types
  const transformedNodes = javaRoadmapData.nodes.map(node => ({
    ...node,
    type: 'default',
    style: {
  ...
  const [edges, setEdges, onEdgesChange] = useEdgesState(javaRoadmapData.edges);
  ...
  const bgGradient = useColorModeValue(
    ...
    'linear(to-br, gray.900, orange.900, red.900)'
  );
    ...
    <Box bgGradient={bgGradient} minH="100vh" p={4}>
      <Container maxW="7xl">
        ...
        <VStack spacing={6} mb={8}>
          <Heading size="2xl" textAlign="center">Java Programming Mastery</Heading>
          ...
          <SimpleGrid columns={{ base: 2, md: 4 }} spacing={6} w="full">
            <Stat textAlign="center">
              ...
              <StatHelpText>
                <CircularProgress value={progressPercentage} color="orange.400" size="60px">
            ...
            <Stat textAlign="center">
              ...
              <StatNumber>Java</StatNumber>
              <StatHelpText>Object-Oriented Programming</StatHelpText>
            </Stat>
